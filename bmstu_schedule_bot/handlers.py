...
def doctor(message):
    ...
    if is_file_exists(configs.CONFIG['doctors']):
        ...
        bot.send_message(
            message.chat.id, text='Расписания врачей у меня временно нет...')
...
def download_schedule(chat_id, group):
    try:
        bmstu_schedule.run(
            group.get(), configs.SEM_START_DATE, configs.CONFIG['vault'])
        return True
    except SystemExit:
...
def is_schedule_exists(group):
    if is_file_exists(configs.SCHEDULE_ICS.format(
        configs.CONFIG['vault'], group)
    ):
        return is_file_exists(
            configs.SCHEDULE_PNG.format(configs.CONFIG['vault'], group))
    return False
